...
function updateCategoriesList() {
    ...
    container.innerHTML = allCategories.map(cat => `
        <div class="bg-white rounded-lg p-4 shadow-sm hover:shadow-md transition flex justify-between items-center">
            <div class="flex-1">
...
function updateProductsList() {
    ...
    container.innerHTML = allProducts.map(p => `
        <div class="bg-white rounded-lg shadow-sm hover:shadow-md transition overflow-hidden flex">
            ...
            <div class="flex-1 p-4">
                ...
                <p class="text-lg font-bold text-orange-600 mt-2">${utils.formatCurrency(p.price)}</p>
    ...
    try {
        ...
        if (imageFile) {
            if (imageFile.size > 5 * 1024 * 1024) {
                utils.notify('⚠️ حجم الصورة كبير جداً (الحد الأقصى: 5MB)', 'error');
                loading.hide();
                return;
            }
...
async function testSupabaseStorage() {
    try {
        const response = await fetch(
            `${SUPABASE_CONFIG.URL}/storage/v1/bucket/restaurant`,
            {
                method: 'GET',
                headers: {
        ...
        if (storageAvailable) {
            try {
                ...
                const uploadResponse = await fetch(
                    `${SUPABASE_CONFIG.URL}/storage/v1/object/restaurant/${fileName}`,
                    {
                        method: 'POST',
                        headers: {
                            'authorization': `Bearer ${SUPABASE_CONFIG.KEY}`,
                ...
                if (uploadResponse.ok) {
                    const publicUrl = `${SUPABASE_CONFIG.URL}/storage/v1/object/public/restaurant/${fileName}`;
...
async function uploadLogo() {
    ...
    if (file.size > 5 * 1024 * 1024) {
        utils.notify('⚠️ حجم الملف كبير جداً (الحد الأقصى: 5MB)', 'error');
        return;
    }
